...
Fundamentally OptVL is an aircraft aerodynamic assessment tool geared towards conceptual designers, multidisciplinary design optimization (MDO) practitioners, and students.
OptVL is based on the popular Athena Vortex Lattice (AVL) code [@DrelaYoungrenAVL], and thus uses a discrete representation of the vorticity distribution over a lifting surface to model its influence possibly in the presence of a slender body like a fuselage.
...
Despite the widespread use of VLM codes, there still remains a need for an open-source VLM that is easy enough for the average aerospace engineer to set up with complete support for scripting and gradient-based optimization.
...
Despite being originally written in the late 70s [@Miranda1997], Vorlax is still updated and used by researchers today [@Souders2021]. 
Altough Vorlax was originally released as source code, it has been developed under a closed-source model at various companies and universities.
...
VSPAERO is an open-source vortex lattice and panel code with many features tightly integrated into open Vehicle Sketch Pad (OpenVSP) geometry tool [@McDonald2022].
It is possible to use its Python API to including OpenVSP and VSPAERO into a larger model.
VSPAERO recently added design sensitives via a handwritten adjoint method that efficiently computes sensitivities of loads.
When combined with the finite differences across the geometric parameterization one can compute the sensitives with respect to the design parameters.
...
OpenAeroStruct [@Jasa2018a] is an open-source Python based aerostructual analysis and optimization tool built with the OpenMDAO framework [@Gray2019a].
...
Like VSPAERO, as of version 2.12.0 it does not offer sensitives of aerodynamic stability derivatives, making it more difficult to ensure the resulting design is statically stable.
Furthermore, 
...
In particular installing X11 (Linux) or XQuartz (macOS) is consistently a stumbling block for students fist learning AVL.
AVL itself is missing features for gradient-based optimization
Additionally, AVL does not compute the sensitives required to integration it into larger gradient based design frameworks.
Only sensitives of the loads with respect to the twist of wing sections are displayed in AVL.
...
Furthermore, it exists as an open-developement project to centralize access to development while inheriting AVL's GPL open-source license.
...
2. Modes of input to specify a geometry without a file and setting aerodynamic meshes directly.
...
This approach was featured in our earlier package, pyAVL (`pyavl-wrapper` on PYPI), which was renamed OptVL to avoid confusion with unrelated projects of similar name and emphasize the support of optimization.
...
For example, to compute the distribution of circulation for nonpenetration flow through the surface the code uses an LU factorization to solve the dense linear system.
...
This is important as optimization is best done explicitly accounting for the impact across many disciplines to avoid unwanted specialization of one discipline at the expense of another, particularly at the conceptual design phase.
...
An aircraft designer at a UAV manufacturer has contributed to the code base, and the authors were invited to present OptVL to engineers at an eVTOL developer evaluating new aircraft concepts.
