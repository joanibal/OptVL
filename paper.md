...
Fundamentally OptVL is a tool for assessing the aerodynamic performance of an aircraft for conceptual designers, multidisciplinary design optimization (MDO) practitioners, and students.
OptVL is based on the popular Athena Vortex Lattice (AVL) code [@DrelaYoungrenAVL], and thus uses a discrete representation of the vorticity distribution over a lifting surface to model its influence.
...
Multidisciplinary design optimization (MDO) is gaining traction within the industry as a complement to traditional design approaches.
...
Despite the widespread use of VLM codes, there still remains a need for an open-source VLM with complete support for scripting, and gradient-based optimization that is easy enough for the average aerospace engineer to set up.
...
Although originally written in the late 70s [@Miranda1997], Vorlax is still updated and used by researchers today [@Souders2021]. 
Altough Vorlax was orginally released as source code, it has been developed under a closed-source model at various companies and universities.
The publicly assessable versions of Vorlax lacks scripting integration and gradients for gradient-based design.

<!-- - tornado
    - matlab based
    - Doesn't have sensitives -->
Tornado is a VLM solver built in MatLab.
...
VSPAERO is an open-source vortex lattice code with many features tightly integrated into OpenVSP [@McDonald2022].
It is possible to use its Python API to including OpenVSP and VSPAERO into a larger model, but one has to download and compile OpenVSP to expose the API.
They have recently developed design sensitives of via the adjoint method, although this is still an active area of development.

OpenAeroStruct is an open-source Python based aerostructual analysis and optimization tool built with the OpenMDAO framework.
...
It couples a vortex-lattice method with structural beam model.
It is fully capable of running and providing sensitives for aero only analysis.
 <!-- did not have all the outputs needed for a practical optimization. Stability derivatives -->
However, it does not offer sensitives of aerodynamic stability derivatives, making it more difficult to ensure the resulting design is statically stable.
<!-- check if this right -->
Furthermore, models are formed through manipulation of the mesh rather than section based geometry definition.


The most prolific VLM solver in the aeronautics field is Athena Vortex Lattice (AVL) [<cite AVL paper>]. 
Users operate AVL by navigating a text interface of setting and parameters via manual text input.
...
However, the text input and output method of traditional AVL wrappers have limitations.
...
In particular install X11 (Linux) or XQuartz (macOS) is consistently a stumbling block for students fist learning AVL. -->
AVL itself is missing features for gradient-based optimization
AVL does not compute the sensitives required to integration it into larger gradient based design frameworks.
AVL only displays the sensitives of the loads with respect to the twist of wing sections.
This leaves missing sensitivity infromation for important wing shape design variables such as the chord distribution and XYZ position of the sections defining the shape.
Finally, The postprocessing of AVL is somewhat limited. 
AVL only IO of the surface data is restricted to an AVL specific format.

AVL is open source but closed development, making it difficult to extend AVL itself to address these shortcomings.
...
Furthermore, it exists as an open-source and open-developement project to centralize access to development.
In addition to maintaining AVL's validated aerodynamics model, OptVL addresses these OptVL has
<!-- Here are the features OptVL has that address those needs -->
1. a Python interface with unified memory that allows one to get and set values in the Fortran level directly.
2. Additional modes of input to specify a geometry without a file and or set aerodynamic meshes directly
    <!-- - ** is this true? We may need to add the body force and section sensitives ** -->
3. Total sensitives of all outputs with respect to all inputs in both forward and reverse mode!
4. Python wheels for all major operating system distributed on PYPI, greatly simplifying distribution
5. IO of aerodynamic surface data in a format readable by industry standard postprocessing tools


# Software design
The design philosophy of OptVL is centered around keeping the code easy to use and understand for aerosapce engineers.
...
Again to keep the level of programming knowledge to a minimum, all the return types from the API are native Python types or NumPy arrays.
By returning the data as Numpy arrays and supplying functions for adding data to Matplotlib frames, a user has a lot more flexibility when deciding how to visualize the results.
An example of a Matplotlib visualization of the coefficient of pressure on the surface of the wing and tail is shown in Figure \autoref{fig:cp}.

![Visualizing the surface pressure distribution in Python via Matplotlib.\label{fig:cp}](cp.png)

The Python layer, `OVSolver`, wraps the underlying Fortran Code.
F2Py is the backbone of the integration between the Python and Fortran layers. 
...
The strategy for creating the adjoint-based sensitivity routines is based on the work of @Kenway2019.
...
This makes it a good fit for engineering design problems where the number of design variables tend to be orders of magnitude greater than the number of functions of interest.
<!-- This required adding routines to explicitly compute a residual for a circulation vector. -->
We use the source-transformation AD tool TAPENADE [@hascoet2013] to create routines for the partial derivatives of the residual and all the outputs with respect to the input geometric, aerodynamic, residual, and state variables.
Each of these partial derivatives routines is tested against finite difference to ensure they are accurate and bug-free. 
These ADed partial derivatives are incorporated into handwritten routines that solve the adjoint system and compute the total derivatives. 
This is important, as one can make simplifications rather than just blindly ADing everything
For example, to compute the distribution of circulation for nonpenetration flow through the surface the code uses an LU factorization to solve the set of elliptic linear PDEs.
It is much more efficient to perform transposed solves with these LU factors rather than use AD on the factorization process. 

To integrate OptVL into an optimization loop a user can create an object and constraint functions that use OVLSolver directly in a SciPy script as shown in the examples.
We also provide an OpenMDAO component that makes it easy to include OptVL as a component of a larger engineering model.
This is really important as optimization is best done explicitly accounting for the impact across many disciplines to avoid unwanted specialization of one discipline at the expense of another, particularly at the conceptual design phase.

<!-- - Although AVL is optionally compiled in double precision and ships single precision binaries, we decided to focus on double precision 
- The extra precision tends to be useful in an optimization context where small steps in the design space can yield comparatively small changes in the output loads
- The extra precision resolves these small differences allowing the optimizer to progress through flat regions of the design space
- finite difference precision tends to fall off like sqrt(function accuracy) -->


<!-- ## CI/CD -->
As part of the effort to make OptVL easy to use we invested time to create Python wheels that could be pip installed across major platforms, macOS arm64, windows x86 and arm64, Linux x86.
...
<!-- Tiwari flying V -->
Furthermore, other researchers have used OptVL when optimizing control surfaces sizes on a novel "flying V" aircraft [@Tiwari2025].
After adding the ability to set OptVL meshes point by point rather than through a geometric definition, @Bakhshi2026 used OptVL with in the context of aerostructural optimization of a <>

In addition to academic use, OptVL has also been used by UAV manufactures.
An aircraft designer from Firestorm has made small contributions to the code base.
Additionally, one author corresponds by email and gave a presentation for an eVTOL manufacturer who is using OptVL to assess new aircraft concepts
...
Jules (Google Gemini 2) was used to correct spelling and grammar in documentation and code comments.
Small features, like adding body visualization based on existing surface visualization, were generated by AI and reviewed and modified by humans.
Additionally, LLM chat tools were used frequently to understand compiler and runtime errors and create helper utilities to manage the common block variables.
Finally, AI was used to provide editorial feedback on first draft of this paper.
