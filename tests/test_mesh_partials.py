...
class TestResidualDPartials(unittest.TestCase):
    def setUp(self):
        self.ovl_solver = OVLSolver(input_dict=input_dict2)
        self.ovl_solver.set_variable("alpha", 25.0)
        self.ovl_solver.set_variable("beta", 5.0)
        self.ovl_solver.execute_run()
    ...
    def test_fwd_mesh(self):
        ...
        for surf_key in self.ovl_solver.surf_mesh_to_fort_var:
            for mesh_key in self.ovl_solver.surf_mesh_to_fort_var[surf_key]:
                ...
                idx_max_abs_error = np.argmax(abs_error)
                # print(
                #     f"{surf_key:10} {mesh_key:10} AD:{np.linalg.norm(res_d_seeds): .5e} FD:{np.linalg.norm(res_d_seeds_FD): .5e} max rel err:{(rel_error[idx_max_rel_error]): .3e} max abs err:{(np.max(abs_error)): .3e}"
                # )
                np.testing.assert_allclose(
                    res_d_seeds,
                    res_d_seeds_FD,
                    atol=1e-4,
                    err_msg=f"deriv_func res w.r.t. {surf_key}:{mesh_key}",
                )
    ...
    def test_rev_mesh(self):
        ...
        mesh_seeds_rev = self.ovl_solver._execute_jac_vec_prod_rev(res_d_seeds=res_d_seeds_rev)[2]
        self.ovl_solver.clear_ad_seeds_fast()
...
class TestConSurfDerivsPartials(unittest.TestCase):
    ...
    def test_fwd_mesh(self):
        ...
        for surf_key in self.ovl_solver.surf_mesh_to_fort_var:
            for mesh_key in self.ovl_solver.surf_mesh_to_fort_var[surf_key]:
                ...
                cs_d = self.ovl_solver._execute_jac_vec_prod_fwd(mesh_seeds={surf_key: {mesh_key: mesh_seeds}})[2]

                cs_d_fd = self.ovl_solver._execute_jac_vec_prod_fwd(
                    mesh_seeds={surf_key: {mesh_key: mesh_seeds}}, mode="FD", step=1e-7
                )[2]
    ...
    def test_rev_geom(self):
        ...
        mesh_seeds_rev = self.ovl_solver._execute_jac_vec_prod_rev(consurf_derivs_seeds=cs_d_rev)[2]
        self.ovl_solver.clear_ad_seeds_fast()
...
class TestBodyAxisDerivDerivsPartials(unittest.TestCase):
    ...
    def test_fwd_mesh(self):
        ...
        for surf_key in self.ovl_solver.surf_mesh_to_fort_var:
            for mesh_key in self.ovl_solver.surf_mesh_to_fort_var[surf_key]:
                ...
                bd_d = self.ovl_solver._execute_jac_vec_prod_fwd(mesh_seeds={surf_key: {mesh_key: mesh_seeds}})[4]

                bd_d_fd = self.ovl_solver._execute_jac_vec_prod_fwd(
                    mesh_seeds={surf_key: {mesh_key: mesh_seeds}}, mode="FD", step=1e-9
                )[4]
    ...
    def test_rev_mesh(self):
        ...
        mesh_seeds_rev = self.ovl_solver._execute_jac_vec_prod_rev(body_axis_derivs_seeds=bd_d_rev)[2]
        self.ovl_solver.clear_ad_seeds_fast()
