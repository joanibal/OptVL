...
C       ELSEIF(COMAND.EQ.'D   ') THEN
C        OVERLAY = .NOT.OVERLAY
C C
C        IF(OVERLAY) THEN
...
C         NFE = MAX( NFE , 1 )
C         WRITE(*,2050) FEVDEF(1:NFE)
C  2050   FORMAT(' Enter eigenvalue data filename: ', A)
C         READ (*,1000) FNNEW
C         IF(FNNEW.NE.' ') FEVDEF = FNNEW
C C
C C------ read the data file
C         CALL EIGINP(FEVDEF, ERROR)
C         IF(ERROR) THEN
C          OVERLAY = .FALSE.
C          GO TO 800
C         ENDIF
C C
C         OVERLAY = .TRUE.
C         WRITE(*,*) 'Overlay plotting enabled'
C C
C         IF(INDEX('P',CHPLOT(1:1)).NE.0) THEN
C          COMAND = CHPLOT
C          GO TO 815
C         ENDIF
C C
C        ELSE
C         WRITE(*,*) 'Overlay plotting disabled'
C C
C        ENDIF
C C
C C-------------------------------------------------------------------
C C---- Zoom in on plot
C       ELSEIF(COMAND.EQ.'Z   ') THEN
C        IF(INDEX('P',CHPLOT(1:1)) .EQ. 0) THEN
C         WRITE(*,*) 'No plot to zoom'
C         GO TO 810
C        ENDIF
C C
...
C C
C C-------------------------------------------------------------------
C C---- Reset zoom on plot
C       ELSEIF(COMAND.EQ.'U   ') THEN
C        CALL CLRZOOM
C        CALL REPLOT(IDEV)
C C
C-------------------------------------------------------------------
