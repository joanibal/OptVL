...
def get_used_variables(ad_src_dir, ad_ext="_DIFF"):
    """Scan all .f files under ad_src_dir and return the set of base variable
    names (uppercased) that appear with the given ad_ext suffix."""
    used = set()
    pattern = re.compile(
        r'\b([A-Za-z][A-Za-z0-9_]*)' + re.escape(ad_ext) + r'\b',
        re.IGNORECASE,
    )
    for root, _, files in os.walk(ad_src_dir):
        for fname in files:
...
def process_fortran_include_file(input_filename, output_filename,
                                  ad_ext="_DIFF", ad_src_dir=None):
    variables = {}
    include_lines = []
    ...
    # Filter to only variables actually used in the AD source files.
    if ad_src_dir is not None:
        used = get_used_variables(ad_src_dir, ad_ext)
        for block in list(variables.keys()):
            before = variables[block]
            variables[block] = [
                v for v in before
                if v.split("(")[0].strip().upper() in used
            ]
            removed = (
                {v.split("(")[0].strip() for v in before}
                - {v.split("(")[0].strip() for v in variables[block]}
            )
            for r in sorted(removed):
                print(f"  skipping unused variable: {r}")
...
if __name__ == "__main__":
    AD_SRC = "../ad_src"
    process_fortran_include_file("AVL.INC", "AVL_ad_seeds.inc", ad_src_dir=AD_SRC)
    process_fortran_include_file("AVL_surf.INC", "AVL_surf_ad_seeds.inc", ad_src_dir=AD_SRC)
