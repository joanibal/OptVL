...
C   variations   of useful results: clff cyff cdff spanef dwwake
...
C   point location
      DO jc=1,nstrip
        ...
        IF (arg1 .EQ. 0.D0) THEN
          dst_diff = 0.D0
        ELSE
          dst_diff = arg1_diff/(2.0*temp)
        END IF
        dst = temp
C
        ny_diff = -((dzt_diff-dzt*dst_diff/dst)/dst)
        ny = -(dzt/dst)
        nz_diff = (dyt_diff-dyt*dst_diff/dst)/dst
        nz = dyt/dst
...
C
C
        dwwake_diff(jc) = -(vy*ny_diff) - ny*vy_diff - vz*nz_diff - nz*
     +    vz_diff
