...
C   variations   of useful results: cdtot_al cltot_al cytot_al
...
C this is the lateral stablity parameter suggested by John Yost
      ...
      ENDDO
      cxtot_u_ba_diff(1) = -cxtot_u_diff(1)
      cxtot_u_ba(1) = -cxtot_u(1)
      cxtot_u_ba_diff(2) = -(dir*cxtot_u_diff(2))
      cxtot_u_ba(2) = -(dir*cxtot_u(2))
      cxtot_u_ba_diff(3) = -cxtot_u_diff(3)
      cxtot_u_ba(3) = -cxtot_u(3)
      DO ii1=1,numax
        cytot_u_ba_diff(ii1) = 0.D0
      ENDDO
      cytot_u_ba_diff(1) = -(dir*cytot_u_diff(1))
      cytot_u_ba(1) = -(dir*cytot_u(1))
      cytot_u_ba_diff(2) = -cytot_u_diff(2)
      cytot_u_ba(2) = -cytot_u(2)
      cytot_u_ba_diff(3) = -(dir*cytot_u_diff(3))
      cytot_u_ba(3) = -(dir*cytot_u(3))
      DO ii1=1,numax
        cztot_u_ba_diff(ii1) = 0.D0
      ENDDO
      cztot_u_ba_diff(1) = -cztot_u_diff(1)
      cztot_u_ba(1) = -cztot_u(1)
      cztot_u_ba_diff(2) = -(dir*cztot_u_diff(2))
      cztot_u_ba(2) = -(dir*cztot_u(2))
      cztot_u_ba_diff(3) = -cztot_u_diff(3)
      cztot_u_ba(3) = -cztot_u(3)
      DO ii1=1,numax
        crtot_u_ba_diff(ii1) = 0.D0
      ENDDO
      crtot_u_ba_diff(1) = -crtot_u_diff(1)
      crtot_u_ba(1) = -crtot_u(1)
      crtot_u_ba_diff(2) = -(dir*crtot_u_diff(2))
      crtot_u_ba(2) = -(dir*crtot_u(2))
      crtot_u_ba_diff(3) = -crtot_u_diff(3)
      crtot_u_ba(3) = -crtot_u(3)
      DO ii1=1,numax
        cmtot_u_ba_diff(ii1) = 0.D0
      ENDDO
      cmtot_u_ba_diff(1) = -(dir*cmtot_u_diff(1))
      cmtot_u_ba(1) = -(dir*cmtot_u(1))
      cmtot_u_ba_diff(2) = -cmtot_u_diff(2)
      cmtot_u_ba(2) = -cmtot_u(2)
      cmtot_u_ba_diff(3) = -(dir*cmtot_u_diff(3))
      cmtot_u_ba(3) = -(dir*cmtot_u(3))
      DO ii1=1,numax
        cntot_u_ba_diff(ii1) = 0.D0
      ENDDO
      cntot_u_ba_diff(1) = -cntot_u_diff(1)
      cntot_u_ba(1) = -cntot_u(1)
      cntot_u_ba_diff(2) = -(dir*cntot_u_diff(2))
      cntot_u_ba(2) = -(dir*cntot_u(2))
C'    roll rate  p','   pitch rate  q','     yaw rate  r'
      cntot_u_ba_diff(3) = -cntot_u_diff(3)
      cntot_u_ba(3) = -cntot_u(3)
      temp = cxtot_u(4)/bref
      cxtot_u_ba_diff(4) = 2.0*(cxtot_u_diff(4)-temp*bref_diff)/bref
      cxtot_u_ba(4) = 2.0*temp
      temp = cxtot_u(5)/cref
      cxtot_u_ba_diff(5) = dir*2.0*(cxtot_u_diff(5)-temp*cref_diff)/cref
      cxtot_u_ba(5) = dir*2.0*temp
      temp = cxtot_u(6)/bref
      cxtot_u_ba_diff(6) = 2.0*(cxtot_u_diff(6)-temp*bref_diff)/bref
      cxtot_u_ba(6) = 2.0*temp
      temp = cytot_u(4)/bref
      cytot_u_ba_diff(4) = dir*2.0*(cytot_u_diff(4)-temp*bref_diff)/bref
      cytot_u_ba(4) = dir*2.0*temp
      temp = cytot_u(5)/cref
      cytot_u_ba_diff(5) = 2.0*(cytot_u_diff(5)-temp*cref_diff)/cref
      cytot_u_ba(5) = 2.0*temp
      temp = cytot_u(6)/bref
      cytot_u_ba_diff(6) = dir*2.0*(cytot_u_diff(6)-temp*bref_diff)/bref
      cytot_u_ba(6) = dir*2.0*temp
      temp = cztot_u(4)/bref
      cztot_u_ba_diff(4) = 2.0*(cztot_u_diff(4)-temp*bref_diff)/bref
      cztot_u_ba(4) = 2.0*temp
      temp = cztot_u(5)/cref
      cztot_u_ba_diff(5) = dir*2.0*(cztot_u_diff(5)-temp*cref_diff)/cref
      cztot_u_ba(5) = dir*2.0*temp
      temp = cztot_u(6)/bref
      cztot_u_ba_diff(6) = 2.0*(cztot_u_diff(6)-temp*bref_diff)/bref
      cztot_u_ba(6) = 2.0*temp
      temp = crtot_u(4)/bref
      crtot_u_ba_diff(4) = 2.0*(crtot_u_diff(4)-temp*bref_diff)/bref
      crtot_u_ba(4) = 2.0*temp
      temp = crtot_u(5)/cref
      crtot_u_ba_diff(5) = dir*2.0*(crtot_u_diff(5)-temp*cref_diff)/cref
      crtot_u_ba(5) = dir*2.0*temp
      temp = crtot_u(6)/bref
      crtot_u_ba_diff(6) = 2.0*(crtot_u_diff(6)-temp*bref_diff)/bref
      crtot_u_ba(6) = 2.0*temp
      temp = cmtot_u(4)/bref
      cmtot_u_ba_diff(4) = dir*2.0*(cmtot_u_diff(4)-temp*bref_diff)/bref
      cmtot_u_ba(4) = dir*2.0*temp
      temp = cmtot_u(5)/cref
      cmtot_u_ba_diff(5) = 2.0*(cmtot_u_diff(5)-temp*cref_diff)/cref
      cmtot_u_ba(5) = 2.0*temp
      temp = cmtot_u(6)/bref
      cmtot_u_ba_diff(6) = dir*2.0*(cmtot_u_diff(6)-temp*bref_diff)/bref
      cmtot_u_ba(6) = dir*2.0*temp
      temp = cntot_u(4)/bref
      cntot_u_ba_diff(4) = 2.0*(cntot_u_diff(4)-temp*bref_diff)/bref
      cntot_u_ba(4) = 2.0*temp
      temp = cntot_u(5)/cref
      cntot_u_ba_diff(5) = dir*2.0*(cntot_u_diff(5)-temp*cref_diff)/cref
      cntot_u_ba(5) = dir*2.0*temp
C control surfaces
      temp = cntot_u(6)/bref
      cntot_u_ba_diff(6) = 2.0*(cntot_u_diff(6)-temp*bref_diff)/bref
